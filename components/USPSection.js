...
//     if (scrollProgress < 0.20 || scrollProgress >= 0.26) {
...
//     const progress = Math.min(1, enterStart / 0.25);
//     const scale = progress;
//     const opacity = progress;
//     return { transform: `scale(${scale})`, opacity };
//   };
...
export default function USPSection() {
  const sectionRef = useRef(null);
  const [scrollProgress, setScrollProgress] = useState(0);
  const [smoothProgress, setSmoothProgress] = useState(0); // <-- Smooth version
  ...
  // Smooth interpolation toward scrollProgress
  useEffect(() => {
  ...
  // --- Animation logic uses smoothProgress instead ---
  const getImage1Transform = () => {
    const progress = Math.min(1, smoothProgress / 0.15);
    const translateX = 100 - progress * 100;
    return { transform: `translateX(${translateX}%)`, opacity: 1 };
  };

  const getImage2Transform = () => {
    const startProgress = Math.max(0, smoothProgress - 0.3);
    const progress = Math.min(1, startProgress / 0.2);
    const translateX = 100 - progress * 100;
    return { transform: `translateX(${translateX}%)`, opacity: 1 };
  };
  ...
  const getImage3Transform = () => {
    const startProgress = Math.max(0, smoothProgress - 0.65);
    const progress = Math.min(1, startProgress / 0.2);
    const translateX = 100 - progress * 100;
    return { transform: `translateX(${translateX}%)`, opacity: 1 };
  };
  ...
  const getText1Transform = () => {
    if (smoothProgress < 0.15 || smoothProgress >= 0.3) {
      return { transform: "scale(0)", opacity: 0 };
    }
    const enterStart = smoothProgress - 0.15;
    const progress = Math.min(1, enterStart / 0.08);
    return { transform: `scale(${progress})`, opacity: progress };
  };

  const getText2Transform = () => {
    if (smoothProgress < 0.5 || smoothProgress >= 0.65) {
      return { transform: "scale(0)", opacity: 0 };
    }
    const enterStart = smoothProgress - 0.5;
    const progress = Math.min(1, enterStart / 0.08);
    return { transform: `scale(${progress})`, opacity: progress };
  };

  const getText3Transform = () => {
    if (smoothProgress < 0.75) return { transform: "scale(0)", opacity: 0 };
    const enterStart = smoothProgress - 0.75;
    const progress = Math.min(1, enterStart / 0.25);
    return { transform: `scale(${progress})`, opacity: progress };
  };
